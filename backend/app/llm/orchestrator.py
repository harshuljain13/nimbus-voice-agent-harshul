...
Phase 2 slice: batch, single-turn, knowledge = RAGless (`context.md`) or none. Returns the
reference-shaped `{text, latency, meta}`. RAG (`use_rag`), Gemini, streaming, history, and tools
are guarded off until their phases and light up the corresponding controls then.
"""
...
def _resolve_model(model_key: str) -> dict[str, str]:
    ...
    if spec["provider"] != "openai":
        raise ChatError(400, f"Provider {spec['provider']!r} arrives in Phase 5 — Phase 2 is OpenAI only.")
    return spec


def _knowledge_mode(use_context: bool, use_rag: bool) -> str:
    if use_rag:
        raise ChatError(400, "RAG retrieval arrives in Phase 3-4 — use RAGless or None for now.")
    return "ragless" if use_context else "none"
...
def assemble(
    ...
    *,
    ...
    """Build the message list + grounding metadata (shared by chat and the inspector)."""
    knowledge = _knowledge_mode(use_context, use_rag)
    context = load_context() if knowledge == "ragless" else ""
    messages = prompts.build_messages(
        message, context, response_length, system_prompt, grounded=(knowledge == "ragless")
    )
    info = {
        "knowledge": knowledge,
...
async def chat(
    *,
    ...
    messages, info = assemble(
        message, response_length=response_length, use_context=use_context,
        use_rag=use_rag, system_prompt=system_prompt,
    )
    ...
    meta = {
        ...
        # placeholders for controls that light up in later phases
        "verbatim_messages": 0,
